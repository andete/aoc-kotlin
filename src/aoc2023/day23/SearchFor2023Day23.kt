...
object SearchFor2023Day23 {
    ...
    fun <E> path(
        ...
        heuristic: (E, List<E>) -> Int = { _,_ -> 0 },
        show: (List<E>) -> Unit = {}
    ): List<E> {
        var result: Pair<Int, List<E>>? = null
        ...
        val fScoreMap = hashMapOf(startNode to 0)
        ...
        while (openSet.isNotEmpty()) {
            ...
            if (current.e == goal) {
                if (result == null || score > result.first) {
                    result = score to reconstruct(current)
                }
                continue
            }
            ...
            for (neighbour in neighboursNodes) {
                ...
                if (tentative > gScore(neighbour)) {
                    ...
                    fScoreMap[neighbour] = tentative + heuristic(neighbour.e, visited)
